    ...
    public class GameBootstrapper : MonoBehaviour
    {
        ...
        private void SetupWorld()
        {
            ...
            if (generatedTerrain != null)
            {
                ...
                // Ensure the terrain has a physics collider — scene terrains that
                // were NOT generated by TerrainGenerator may be missing one.
                var tc = generatedTerrain.GetComponent<TerrainCollider>();
                if (tc == null)
                {
                    tc = generatedTerrain.gameObject.AddComponent<TerrainCollider>();
                    Debug.LogWarning("[GameBootstrapper] Added missing TerrainCollider.");
                }
                // Ensure collider references the correct data
                if (tc.terrainData == null && generatedTerrain.terrainData != null)
                {
                    tc.terrainData = generatedTerrain.terrainData;
                    Debug.LogWarning("[GameBootstrapper] Linked TerrainCollider to terrainData.");
                }
            }
            else
            {
        ...
        private void SetupPlayer()
        {
            ...
            {
                // Player already in scene — remove any Rigidbody that would fight
                // with the CharacterController and cause physics-driven falling.
                var rb = existingPlayer.GetComponent<Rigidbody>();
                if (rb != null)
                {
                    Destroy(rb);
                    Debug.LogWarning("[GameBootstrapper] Removed conflicting Rigidbody from player.");
                }

                // Teleport via CharacterController disable/enable to avoid CC blocking it
                ...
                Debug.LogWarning($"[GameBootstrapper] Existing player repositioned to {spawnPos}");
                return;
            }
        ...
        private Vector3 GetSpawnPositionOnTerrain()
        {
            ...
            if (terrain != null)
            {
                ...
                Debug.LogWarning($"[GameBootstrapper] Spawn on terrain at Y={pos.y} (terrain height={terrainY})");
            }
            else
            {
